...
def get_uniq_id_docs(input_df, uniq_ids):
    docs = list(input_df[input_df["uniq_id"].isin(uniq_ids)]["text"].str.lower())
    return docs


def get_vectoriser_and_train_vectors(input_df, train_ids, stop_words=None):
...
def main():
    ...
    tdf_if_vectoriser, train_X = get_vectoriser_and_train_vectors(data_df, train_ids, stop_words={'english'})
    ...
    test_X = get_tdf_if_vectors(tdf_if_vectoriser, test_docs)
    test_pred_y = tdf_if_clf.predict(test_X)
    test_pred_proba_y = tdf_if_clf.predict_proba(test_X)

    un, up, tp, fp, tn, fn = 0, 0, 0, 0, 0, 0
    for val_id, y, pred_y, pred_proba_y in zip(test_ids, test_y, test_pred_y, test_pred_proba_y):
        # print(f"{y}\t{pred_y}\t{pred_proba_y}")
        if max(pred_proba_y) < 0.7:
            if y:
                up += 1
